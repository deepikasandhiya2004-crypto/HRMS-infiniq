...
export default function ApplyLeave() {
  ...
  // Form fields
  const [leaveCode, setLeaveCode] = useState(searchParams.get("type") || "CL");
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const [halfDay, setHalfDay] = useState(false);
  const [halfDaySession, setHalfDaySession] = useState("First Half");
  const [reason, setReason] = useState("");
  const [approver, setApprover] = useState("Vikram Malhotra");
  ...
  const validateForm = () => {
    ...
    if (selectedType?.code === "SL" && requestedDays >= 2 && !attachment) {
      errs.attachment = "Medical proof / doctor prescription is required for Sick Leave of 2+ days";
    }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <option value="Priya Nair">Priya Nair (HR Operations Head)</option>
                <option value="Deepika Sandhiya">Deepika Sandhiya (CHRO)</option>
              </select>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-2xl bg-[#00373A]/5 p-4 text-xs text-slate-600 border border-[#00373A]/10 space-y-2">
            ...
            <p className="text-[11px] leading-relaxed">
              Once submitted, your application will appear under <strong>My Leave</strong> with an initial status of <strong>Pending</strong>. Your manager ({approver}) will be notified for review.
            </p>
          </div>
        </div>
      </form>
