...
import { Coffee, LogIn, LogOut, PlayCircle, PauseCircle, Search, CalendarDays, UserCheck, Palmtree, AlertTriangle, Clock } from "lucide-react";
...
export default function Attendance() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white p-5 border border-primary/10 shadow-sm">
        ...
        {today ? (
          <div className="flex flex-wrap items-center gap-4 text-xs text-primary/70">
            <span>
              Check-in:{" "}
              <b className="text-primary">{fmtTime(today.check_in)}</b>
            </span>
            <span>
              Check-out:{" "}
              <b className="text-primary">{fmtTime(today.check_out)}</b>
            </span>
            <span>
              Worked:{" "}
              <b className="text-primary">{fmtDuration(today.work_seconds)}</b>
            </span>
            <span>
              Break:{" "}
              <b className="text-primary">{fmtDuration(today.break_seconds)}</b>
            </span>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {!today && (
            <button
              disabled={busy}
              onClick={() => action(() => api.post("/attendance/check-in"))}
              className="flex items-center gap-1.5 rounded-lg bg-accent px-3 py-2 text-xs font-bold text-primary disabled:opacity-60"
            >
          ...
          {today && !today.check_out && !today.on_break && (
            <button
              disabled={busy}
              onClick={() => action(() => api.post("/attendance/break/start"))}
              className="flex items-center gap-1.5 rounded-lg bg-purple/10 px-3 py-2 text-xs font-bold text-purple disabled:opacity-60"
            >
          ...
          {today && today.on_break && (
            <button
              disabled={busy}
              onClick={() => action(() => api.post("/attendance/break/end"))}
              className="flex items-center gap-1.5 rounded-lg bg-purple px-3 py-2 text-xs font-bold text-white disabled:opacity-60"
            >
          ...
          {today && !today.check_out && (
            <button
              disabled={busy}
              onClick={() => action(() => api.post("/attendance/check-out"))}
              className="flex items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-xs font-bold text-white disabled:opacity-60"
            >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <input
            ...
            className="rounded-lg border border-primary/20 px-2 py-1.5 text-xs"
          />
        </div>
        ...
        {history && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <StatCard
              title="Present"
              value={history.summary.present_days}
              icon={UserCheck}
              accentColor="#00DC46"
            />
            <StatCard
              title="WFH"
              value={history.summary.wfh_days}
              icon={CalendarDays}
              accentColor="#7C3AED"
            />
            <StatCard
              title="On Leave"
              value={history.summary.leave_days}
              icon={Palmtree}
              accentColor="#FF6A3D"
            />
            <StatCard
              title="Missed Checkouts"
              value={history.summary.missed_checkouts}
              icon={AlertTriangle}
              accentColor="#FF6A3D"
            />
            <StatCard
              title="Avg. Hours / Day"
              value={fmtDuration(avgHours)}
              icon={Clock}
              accentColor="#00373A"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 p-4 border-b border-primary/10">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-primary/15 pl-8 pr-3 py-2 text-xs"
              />
            </div>
            <Dropdown
              value={statusFilter}
              ...
              <tbody>
                {filteredRecords.map((r) => (
                  <tr key={r.id} className="border-b border-primary/5">
                    ...
                    <td className="py-2.5 px-4 text-primary/70">
                      {fmtTime(r.check_in)}
                    </td>
                    <td className="py-2.5 px-4 text-primary/70">
                      {fmtTime(r.check_out)}
                    ...
                    <td className="py-2.5 px-4 text-primary/70">
                      {fmtDuration(r.break_seconds)}
                    </td>
                    <td className="py-2.5 px-4">
                      <StatusBadge status={r.status} size="sm" />
                      {r.missed_checkout && (
                        <span className="ml-1 text-warning text-[10px] font-bold">
                          missed checkout
                        </span>
                      )}
                    </td>
                ...
                {filteredRecords.length === 0 && (
                  <tr>
                    <td
                      colSpan={6}
                      className="py-6 text-center text-primary/40"
                    >
                      No records found.
                    </td>
                  </tr>
      ...
      <div className="rounded-2xl bg-white p-5 border border-primary/10 shadow-sm">
        <h2 className="text-sm font-extrabold text-primary mb-3">
          Request Correction
        </h2>
        <form
        ...
        >
          <input
            ...
            className="rounded-lg border border-primary/20 px-2 py-1.5"
          />
          <select
          ...
          <button
            disabled={busy}
            type="submit"
            className="sm:col-span-2 rounded-lg bg-primary py-2 text-xs font-bold text-white disabled:opacity-60"
          >
            Submit Request
          </button>
        </form>
              ...
              >
                <div>
                  <p className="font-semibold text-primary">
                    {fmtDate(r.work_date)} — {r.issue_type.replace(/_/g, " ")}
                  </p>
      ...
      {isReviewer && (
        <div className="rounded-2xl bg-white p-5 border border-primary/10 shadow-sm">
          ...
          {pending.length === 0 && (
            <p className="text-xs text-primary/50">Nothing pending.</p>
          )}
          <div className="divide-y divide-primary/5">
            {pending.map((r) => (
              ...
              >
                <div>
                  <p className="font-semibold text-primary">
                    {r.employee_name} · {fmtDate(r.work_date)} ·{" "}
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    className="rounded-lg bg-accent px-3 py-1.5 font-bold text-primary"
                  >
                    Approve
                  </button>
                  <button
                    ...
                    className="rounded-lg bg-red-100 px-3 py-1.5 font-bold text-red-600"
                  >
                    Reject
                  </button>
                </div>
